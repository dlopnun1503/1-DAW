package TEMA7.Teoria;

import java.io.File;
import java.io.FileReader;
import java.io.FileWriter;
import java.io.IOException;
import java.nio.charset.StandardCharsets;
import java.util.Scanner;

public class EscrituraLectura {

    public static void main(String[] args) {


        // VAMOS A ESCRIBIR EN UN FICHERO COSAS HASTA QUE SE ESCRIBA FIN
        // 0º Obtener la barra separadora
        String ls = File.separator;

        // 1º Abrir el fichero
        File ficheroPrueba = new File("resources/archivosTema7/Fichero_prueba.txt");

        // 2º Comprobar que el fichero existe
        if(ficheroPrueba.exists() && ficheroPrueba.isFile() && ficheroPrueba.canWrite()) {

            // 3º Abrir flujos (en este caso, de escritura) -> Lanza IOException
            FileWriter fw = null;
            try {
                fw = new FileWriter(ficheroPrueba);

                String cadenaAEscribir = "";
                Scanner scan = new Scanner(System.in);
                while (!cadenaAEscribir.equalsIgnoreCase("fin")) {

                    System.out.println("Oh Elron, dime algo bonito...");
                    cadenaAEscribir = scan.nextLine();

                    if(!cadenaAEscribir.equalsIgnoreCase("fin")) {
                        // 4º Operar con el fichero
                        fw.write(cadenaAEscribir);
                    }
                }

                // 5º Cerrar los flujos
                fw.close();


            } catch (IOException e) {
                e.printStackTrace();
                System.out.println("Error al operar con el fichero");
                // Para cerrar el flujo en caso de error
                if (fw != null) {
                    try {
                        fw.close();
                    } catch (Exception ex){
                        e.printStackTrace();
                    }
                }
            }

        }


        //Pedir usuario y contraseña hasta que escriba fin
        // 1º Abrir el fichero
        File usuario = new File("resources/archivosTema7/usuario.txt");

        // 2º Comprobar que el fichero existe
        if(usuario.exists() && usuario.isFile() && usuario.canWrite()) {

            // 3º Abrir flujos (en este caso, de escritura) -> Lanza IOException
            FileWriter fw = null;
            try {
                fw = new FileWriter(usuario);

                String user = "";
                String pass = "";
                Scanner scan = new Scanner(System.in);
                while (!user.equalsIgnoreCase("fin")) {

                    System.out.println("Escribe tu usuario");
                    user = scan.nextLine();

                    if(!user.equalsIgnoreCase("fin")) {
                        System.out.println("Escribe la contrasenia");
                        pass = scan.nextLine();
                    }
                    if(!user.equalsIgnoreCase("fin")) {
                        // 4º Operar con el fichero
                        fw.write(user+":"+pass);
                    }
                }

                // 5º Cerrar los flujos
                fw.close();


            } catch (IOException e) {
                e.printStackTrace();
                System.out.println("Error al operar con el fichero");
                // Para cerrar el flujo en caso de error
                if (fw != null) {
                    try {
                        fw.close();
                    } catch (Exception ex){
                        e.printStackTrace();
                    }
                }
            }

        }


        //AHORA LECTURA DE FICHEROS
        // 1º Abrir el fichero
        File ficheroParaLeer = new File("resources/archivosTema7/Fichero_prueba.txt");

        //2º Comprobar si existe
        if(ficheroParaLeer.exists() && ficheroParaLeer.isFile() && ficheroParaLeer.canRead()){

            //3º Abrir flujos de lectura -> Lanza IO exception
            FileReader fr = null;
            try {
                fr = new FileReader(ficheroParaLeer);

                //4º Operar con el fichero
                int caracter = 0;
                while (caracter != -1){ //MIENTRAS SEA DISTINTO DE -1 SEGUIMOS LEYENDO
                    caracter = fr.read(); // fr.read devuelve un entero... El caracter en unicode

                    System.out.print((char)caracter); //(char)caracter lo cambia de numero a caracter
                }

                //5º Cerramos el flujo
                fr.close();


            }catch (IOException e){
                e.printStackTrace();
                System.out.println("Error al operar con el fichero");
            }
        }


        // 1º Abrir el fichero
        File ficheroParaLeer2 = new File("resources/archivosTema7/usuario.txt");

        //2º Comprobar si existe
        if(ficheroParaLeer2.exists() && ficheroParaLeer2.isFile() && ficheroParaLeer2.canRead()){

            //3º Abrir flujos de lectura -> Lanza IO exception
            FileReader fr = null;
            try {
                fr = new FileReader(ficheroParaLeer2);

                //4º Operar con el fichero
                int caracter = 0;
                String user = "";
                String pass = "";
                String cadena = "";
                while (caracter != -1){ //MIENTRAS SEA DISTINTO DE -1 SEGUIMOS LEYENDO
                    caracter = fr.read(); // fr.read devuelve un entero... El caracter en unicode
                    if (caracter != 10){
                        cadena += (char)caracter;
                    }else {
                        user = cadena.split(":")[0];
                        pass = cadena.split(":")[1];
                        System.out.println("User: "+user);
                        System.out.println("Pass: "+pass);
                        cadena = "";
                    }

                    System.out.print((char)caracter); //(char)caracter lo cambia de numero a caracter
                }

                //5º Cerramos el flujo
                fr.close();


            }catch (IOException e){
                e.printStackTrace();
                System.out.println("Error al operar con el fichero");
            }
        }


    }
}